...
The **First Optical Enclosure (FOE)** contains the instrumentation used to process the X-rays
generated by the (WW storage ring) so that they can be used for a
particular experiment at a (WW beamline).  While each kind of experiment
has special requirements and may have a different complement of optics, the
FOE shown here is typical.
